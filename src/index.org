...
**** Data Klassen
     In Anwendungsfällen wie z.B. Datentransferobjekte (DTO) benötigt
     man eine Klasse nur, um Daten zu transportieren, aber Logik muss
     in der Klasse so gut wie gar nicht untergebracht werden. Daher
     gibt es in Kotlin die ~data classes~, die im ~primary
     constructor~ die Properties der Klasse aufführen, für die dann
     automatisiert Code für ~toString()~, ~equals()~, ~hashCode()~,
     ~copy()~ und die Komponenten Funktionen für die Unterstützung von
     Destructuring Declarations erzeugt wird.

     #+begin_src kotlin :exports both
       data class MyData(val aMember : Int, val bMember : Double)
       val myData = MyData(4,45.0)
       println(myData) // hier greift die erzeugte toString() Methode
     #+end_src

     #+RESULTS:
     : MyData(aMember=4, bMember=45.0)

     Eine ~copy()~ Funktion kann genutzt werden, um in sehr kurzer Form
     eine Kopie eines bestehenden Objekts anzulegen und bei Bedard dabei wenige
     Properties zu ändern.
#+begin_src kotlin :exports both
data class MyData(val aMember : Int, val bMember : Double)
val objMyData = MyData(4, 4.5)
val copiedMyData = objMyData.copy(bMember = 5.5)
println(copiedMyData)
#+end_src

#+RESULTS:
: MyData(aMember=4, bMember=5.5)
